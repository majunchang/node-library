<template>
    <div class="page-content">
        <input type="search" class='search-input' placeholder='请输入想要查询书籍的书名或者作者名' v-model='searchInput' v-on:keyup='searchBooks'>
        <Table border :columns="columns5" :data="data5"></Table>
        <Page :total="totalNum" show-elevator show-total :current='current' :page-size='pageSize' @on-change='changePageNum'></Page>
        <Modal v-model='updateModal' title="更新提示框" @on-ok="confirmUpdate" @on-cancel="cancelUpdate">
            <p>您确定要更新<<{{updateBookName}}>>这本书吗</p>
        </Modal>
        <Modal v-model='RemoveModal' @on-ok="confirmRemove" @on-cancel="cancelRemove" width="360">
            <p slot="header" style="color:#f60;text-align:center">
                <Icon type="information-circled"></Icon>
                <span>删除确认</span>
            </p>
            <p>您确定要删除
                <<{{RemoveBookName}}>>这本书吗</p>
            <div slot="footer">
        ...
        <Modal v-model='borrowModal' title="借书提示框" @on-ok="confirmBorrow" @on-cancel="cancelBorrow">
            <!--在这里 使用vuex获取到当前登录用户-->
            <p>您确定要借
                <<{{borrowBookName}}>>这本书吗</p>
        </Modal>
    </div>
</template>
